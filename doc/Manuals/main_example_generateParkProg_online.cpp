...
/// @brief program for generate online a parking program
...
int main(int argc, char *argv[])
{
    try {
        ...
        //-------------------------------------------------------
        //Get the Bid from file type FMOSA.
        ...
        //In the MCS you get the Bid also from the file type FMOSA, but it is possible it was already in memory.
        //For generate a parking program, the FMOSA is used only for get Bid.
        ...
        //-------------------------------------------------------
        //Call the function generateParkProg_online:

        bool parkProgSuitable;      //true: if the generated parking program is suitable to be executed.

        //A parking program is suitable to be executed when avoid collissions,
        //even if there are collided or obstructed RPs (in the initial status).

        std::cout << "Calling function generateParkProg_online(outputs, FMM, p_1s_actual, p___3s_actual, RPids_disabled_actual, Bid)..." << std::endl;

        //Generate the parking program online.
        parkProgSuitable = generateParkProg_online(outputs, FMM, p_1s_actual, p___3s_actual, RPids_disabled_actual, Bid);

        std::cout << "Returned from function generateParkProg_online." << std::endl;
        std::cout << "ParkProgSuitable: " << BoolToStr(parkProgSuitable, true).str << std::endl;

        //-------------------------------------------------------
        //Use the output data for program the real Fiber MOS:

        //Definitions:
        //  outputs.ParkProg:                   the motion program containing the instructions
        //  outputs.ParkProg.getcount():        number of gestures to be exxecuted
        //  outputs.ParkProg[i].getCount():     number of message-of-instructions in the gesture i
        //  outputs.ParkProg[i][j].Instruction: the instructions contained in the message (i, j)
        ...
        TMotionProgram *ParkProg = &outputs.ParkProg;

        std::cout << std::endl;
        std::cout << "Accessing to the ParkProg (Parking Program):" << std::endl;
        ...
        std::cout << std::endl;
        std::cout << "Printing the parking program:";
