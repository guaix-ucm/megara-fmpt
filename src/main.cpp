...
/*    str += "\r\n";
    ...
    str += "\r\n    (The format FMPT use absolute coordinates and own instruction).";
    str += "\r\n";
    ...
    str += "\r\n";
    str += "\r\n    outputs: force save file outputs in formats MEG and JSON, even if there are dangerous RPs.";
    ...
    str += "\r\n    ad-hoc old format (called format MEG), is requested for other functions which";
...
string help_others(void)
{
    ...
    str += "\r\n    <PP_path>: absolute or relative path to file containing the PP in the format MCS.";
                              ...
                              bool saveOutputs, bool saveOtherOutputs)
{
    try {
        ...
        if(suitable) {
            //save outputs in format MEG, if any
            if(saveOutputs) {
                outputs.getTextMEG(str);
                output_path = output_dir + "/ParkProg_outputs_from_" + stem(filename) + ".meg";
                strWriteToFile(output_path, str);
                append("Outputs in format MEG (not including FMOSA) saved in file '"+output_path+"'.", log_path.c_str());
            }

            //save outputs in format JSON
            outputs.getTextJSON(str);
    ...
    try {
        ...
        do {
            ...
            if(ParkProgValid) {
                ...
                ParkProg.getMCStext(str, "depos", Bid, SPL);
                append("Parking program translated to the format MCS.", log_path.c_str());
                output_path = output_dir+"/ParkProg_MCS_from_"+filename;
                strWriteToFile(output_path, str);
                append("Parking Program in format MCS saved in file '"+output_path+"'.", log_path.c_str());
            }
                              ...
                              bool saveMCS, bool saveOutputs, bool saveOtherOutputs)
{
    try {
        ...
        if(suitable) {
            ...
            if(saveOutputs) {
                outputs.getTextMEG(str);
                output_path = output_dir + "/PairPPDP_outputs_from_" + stem(filename) + ".meg";
                strWriteToFile(output_path, str);
                append("Outputs in format MEG (including FMOSA) saved in file '"+output_path+"'.", log_path.c_str());
            }
    ...
    try {
        ...
        do {
            ...
            if(PairPPDPvalid) {
                ...
                PP.getMCStext(str, "pos", Bid, IPL);
                append("Positiong program translated to the format MCS.", log_path.c_str());
                output_path = output_dir+"/PairPPDP_PP-MCS_from_"+filename;
                strWriteToFile(output_path, str);
                append("PP in format MCS saved in file '"+output_path+"'.", log_path.c_str());
                ...
                DP.getMCStext(str, "depos", Bid, OPL);
                append("Depositiong program translated to the format MCS.", log_path.c_str());
                output_path = output_dir+"/PairPPDP_DP-MCS_from_"+filename;
                strWriteToFile(output_path, str);
                append("DP in format MCS saved in file '"+output_path+"'.", log_path.c_str());
                ...
                //  //save outputs in format MEG
                //  outputs.getTextMEG(str, Bid, OPL, IPL, true);
                //  output_path = output_dir+"/PairPPDP_outputs_from_"+filename;
                //  strWriteToFile(output_path, str);
                //  append("Pair (PP, DP) in format MCS, saved in file '"+output_path+"'.", log_path.c_str());

                //save outputs in format JSON
                str = outputs.getTextJSON();
                output_path = output_dir + "/PairPPDP_outputs_from_" + stem(filename) + ".json";
                strWriteToFile(output_path, str);
    ...
    try {
        ...
        if(PPvalid) {
            ...
            if(DPvalid) {
                ...
                //save outputs in format MEG
                outputs.getTextMEG(str, true);
                output_path = output_dir + "/PairPPDP_outputs_from_" + stem(filename) + ".meg";
                strWriteToFile(output_path, str);
                append("Pair (PP, DP) in format MEG saved in file '"+output_path+"'.", log_path.c_str());
